...
clusplot(datatovisualize, fit$cluster,cex=1,xlab=colnames(datatovisualize)[1],ylab=colnames(datatovisualize)[2],col.p=fit$cluster,lines=0,labels=1)
...
datatovisualize$quality <- raw$quality
#put cluster back into raw data so we can save raw data and sue in shiny app
...
for(i in 1:10){
  boxplot(datatovisualize[,i] ~ fit$cluster,
        xlab='Cluster Number', ylab=colnames(datatovisualize)[i],
        main=paste('Clusters of ', as.character(colnames(datatovisualize)[i])))
}
...
data_scaled$cluster_hclust <- groups
